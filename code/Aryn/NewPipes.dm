	...
	freezer
		...
		attack_hand(mob/user as mob)
			...
			if(Network)
				...
				var/dat = {"<DOCTYPE html><B>Cryo gas cooling system</B><BR>
		...
		Topic(href, href_list)
			if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon/ai)))
				...
				if(href_list["safety-off"])
					var/message =	"<DOCTYPE html>Warning! The limits are there for a reason, and bypassing them may damage the equipment. Without them, it is possible to change the temperature too quickly, or to values outside of the safe operating range, risking equipment damage.<br><br>"
					message +=		"<A href='?src=\ref[src];safety-off-confirm=1'>Disable limits</A> (Note: requires officer ID)<!html>"
				...
				if(href_list["safety-off-confirm"])
					if(usr.has_access(list(access_heads)))
						...
						message +=		"<FONT COLOR=red>Inadequate ID!</FONT><!html>"
						usr << browse(message, "window=freezerlimits")
