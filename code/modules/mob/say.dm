...
/mob/proc/say_quote(var/text, var/mode)
	...
	if (stuttering)
		return "[pick("заикается","запинается","мямлит","лепечет")], \"[text]\"";
	if (brainloss >= 60)
		return "[pick("мычит","бубнит","стонет","слюнявит")], \"[text]\"";
	if(mode && mode != "")
		return "[mode], \"[text]\""
	if (ending == "?")
		return "[pick("спрашивает","вопрошает","интересуется","осведомляется","распрашивает")], \"[text]\"";
	if (ending == "!")
		return "[pick("вскрикивает","выкрикивает","восклицает")], \"[text]\"";
