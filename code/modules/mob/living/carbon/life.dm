...
/mob/living/carbon/Life()
	...
	//Pain handling (только для людей, для остальных — пустой прок)
	handle_pain()
...
// ===== БАЗОВЫЙ ПРОК handle_pain() ДЛЯ ВСЕХ CARBON =====
/mob/living/carbon/proc/handle_pain()
	// Базовая реализация — ничего не делает
	// Переопределяется в /mob/living/carbon/human/handle_pain()
	return

// ===== БАЗОВЫЙ ПРОК pain() ДЛЯ ВСЕХ MOB (кроме людей) =====
/mob/proc/pain(var/partname, var/amount, var/force)
	// Базовая реализация для не-человеческих мобов
	if(stat != 0) return // STAT_ALIVE = 0 в старых билдах
...
/mob/living/carbon/proc/handle_virus_updates()
	...
	if(!reagents.has_reagent("spaceacillin"))
		if(!microorganism)
			...
			if(microorganism && microorganism.infectionchance)
...
/mob/living/carbon/proc/handle_environment(datum/gas_mixture/environment)
	...
	if((bodytemperature > (T0C + 50)) || (bodytemperature < (T0C + 10)) && (!istype(loc, /obj/machinery/atmospherics/unary/cryo_cell)))
...
/mob/living/carbon/proc/handle_breath(datum/gas_mixture/breath)
	...
	if(Toxins_pp > safe_toxins_max)
		...
		if(toxs >= safe_toxins_max)
			...
			toxloss += min(ratio*vsc.plc.PLASMA_DMG, 10*vsc.plc.PLASMA_DMG)
	...
	if(breath.temperature > (T0C+66) && !(mutations & 2))
		if(prob(20))
...
/mob/living/carbon/proc/adjust_body_temperature(current, loc_temp, boost)
	...
	var/difference = abs(current-loc_temp)
	var/increments
	if(difference > 50)
		increments = difference/5
	else
		increments = difference/10
	var/change = increments*boost
	var/temp_change
	if(current < loc_temp)
