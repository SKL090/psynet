...
/mob/living/carbon/proc/UpdateDamage()
	...
	for(var/datum/organ/external/O in L)
		// Только торс и голова дают полный урон в здоровье
		if(O.name in list("chest", "head", "groin"))
			bruteloss += O.get_damage_brute()
			fireloss += O.get_damage_fire()
		// Конечности — только 20% в общий пул
		else
			bruteloss += O.get_damage_brute() * 0.2
			fireloss += O.get_damage_fire() * 0.2
