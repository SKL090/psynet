...
/mob/var/obj/screen/zone_sel/zone_sel2 = null
...
/obj/screen/zone_sel/MouseDown(location, control, params)
	...
	var/new_selecting = old_selecting

	if(name == "zone_sel2")
	// ===== ZONE_SEL2.DMI — НИЖНЯЯ ПОЛОВИНА =====
		if(icon_x >= 3 && icon_x <= 7 && icon_y >= 29 && icon_y <= 32)
			new_selecting = "r_hand"
		else if(icon_x >= 24 && icon_x <= 28 && icon_y >= 29 && icon_y <= 32)
			new_selecting = "l_hand"
		else if(icon_x >= 5 && icon_x <= 14 && icon_y >= 0 && icon_y <= 4)
			new_selecting = "r_foot"
		else if(icon_x >= 17 && icon_x <= 26 && icon_y >= 0 && icon_y <= 4)
			new_selecting = "l_foot"
		else if(icon_x >= 7 && icon_x <= 14 && icon_y >= 9 && icon_y <= 20)
			new_selecting = "r_leg"
		else if(icon_x >= 19 && icon_x <= 24 && icon_y >= 9 && icon_y <= 20)
			new_selecting = "l_leg"
		else if(icon_x >= 13 && icon_x <= 18 && icon_y >= 18 && icon_y <= 22)
			new_selecting = "groin"
		else if(icon_x >= 10 && icon_x <= 21 && icon_y >= 27 && icon_y <= 32)
			new_selecting = "chest"
		else if(icon_x >= 9 && icon_x <= 22 && icon_y >= 22 && icon_y <= 32)
			new_selecting = "vitals"
	else
		// ===== ZONE_SEL.DMI — ВЕРХНЯЯ ПОЛОВИНА =====
		if(icon_x >= 13 && icon_x <= 14 && icon_y >= 22 && icon_y <= 23)
			new_selecting = "r_eye"
		else if(icon_x >= 17 && icon_x <= 18 && icon_y >= 22 && icon_y <= 23)
			new_selecting = "l_eye"
		else if(icon_x >= 14 && icon_x <= 17 && icon_y >= 18 && icon_y <= 19)
			new_selecting = "mouth"
		else if(icon_x >= 3 && icon_x <= 9 && icon_y >= 1 && icon_y <= 14)
			new_selecting = "r_arm"
		else if(icon_x >= 23 && icon_x <= 28 && icon_y >= 1 && icon_y <= 14)
			new_selecting = "l_arm"
		else if(icon_x >= 11 && icon_x <= 20 && icon_y >= 26 && icon_y <= 29)
			new_selecting = "head"
		else if(icon_x >= 11 && icon_x <= 20 && icon_y >= 18 && icon_y <= 25)
			new_selecting = "face"
		else if(icon_x >= 11 && icon_x <= 20 && icon_y >= 12 && icon_y <= 15)
			new_selecting = "throat"
		else if(icon_x >= 10 && icon_x <= 21 && icon_y >= 1 && icon_y <= 6)
			new_selecting = "vitals"
		else if(icon_x >= 10 && icon_x <= 21 && icon_y >= 7 && icon_y <= 12)
			new_selecting = "chest"

	if(new_selecting != old_selecting)
		selecting = new_selecting

		var/mob/M = usr
		if(M && M.zone_sel)
			M.zone_sel.selecting = selecting
			M.zone_sel.overlays = null
			M.zone_sel.overlays += image("icon" = 'icons/mob/HUD/zone_sel.dmi', "icon_state" = "polovina")
			if(selecting in list("head","face","mouth","r_eye","l_eye","throat","chest","r_arm","l_arm","vitals"))
				M.zone_sel.overlays += image("icon" = 'icons/mob/HUD/zone_sel.dmi', "icon_state" = selecting)

		if(M && M.zone_sel2)
			M.zone_sel2.selecting = selecting
			M.zone_sel2.overlays = null
			M.zone_sel2.overlays += image("icon" = 'icons/mob/HUD/zone_sel2.dmi', "icon_state" = "polovina2")
			if(selecting in list("groin","r_hand","l_hand","r_leg","l_leg","r_foot","l_foot","vitals","chest"))
				M.zone_sel2.overlays += image("icon" = 'icons/mob/HUD/zone_sel2.dmi', "icon_state" = selecting)
