...
mob/new_player
	...
	Login()
		...
		spawn(25)
			new_player_panel()
		src << "<b>Что не насмерть, то не в счет.<!b>"
		...
		startup = sound('music/bumtown.ogg', volume = 30)
		spawn(25)
			src << startup
	...
	verb
		new_player_panel()
			...
			var/output = "<!DOCTYPE html><head></head><body><HR><B>New Player Options</B><BR>"
			...
			output += "<BR><a href='byond://?src=\ref[src];observe=1'>Observe</A><BR></body></html>"
