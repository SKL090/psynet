...
/obj/admins/Topic(href, href_list)
	if(href_list["gurps_stats"])
		var/mob/living/carbon/human/H = locate(href_list["gurps_stats"])
		if(istype(H))
			gurps_edit_player(H)
		return
	..()
...
/obj/admins/proc/player()
	...
	for(var/mob/M in mobs)
		if(M.ckey)
			...
			//GURPS
			if(istype(M, /mob/living/carbon/human))
				dat += "<td align=center><A HREF='?src=\ref[src];gurps_stats=\ref[M]'>GURPS</A></td>"
...
/obj/admins/proc/Game()
	...
	dat += "<!DOCTYPE html><center><B>Game Panel</B></center><hr>\n"
...
/obj/admins/proc/deadmin()
	if(alert(usr, "Подтвердите де-администрирование? Вернуть должность себе на этот раунд не получится", "Подтверждение", "Да", "Нет") == "Да")
		log_admin("[usr.ckey] deadmined themself.")
		message_admins("[usr.ckey] deadmined themself.", 1)

		admins.Remove(usr.ckey)

		if(usr.client)
			usr.client.clear_admin_verbs()

		usr << "<span class='interface'>Теперь вы лишь игрок.</span>"
		del(src)
	return 1


/obj/admins/proc/remove_admin_verbs()
	var/client/C = owner
	if(!C && usr && usr.client)
		C = usr.client
	...
	var/list/averbs_coder = list(
			...
			/client/proc/ticklag,
			/proc/release)
	C.verbs -= averbs_coder
	C.verbs -= averbs_super_admin
	C.verbs -= averbs_primary_admin
	C.verbs -= averbs_admin
	C.verbs -= averbs_secondary_admin
	C.verbs -= averbs_moderator
